...
export class StatusesService
{
    ...
    private _typeStatuses: BehaviorSubject<TypeStatus[] | null> = new BehaviorSubject(null);
        ...
        return this._httpClient.get<Status[]>('http://localhost:1616/api/v1/followup/statuses/all', {
            ...
            tap((statuses) => {
                ...
                if ( query )
                {
                    ...
                    statusFiltered = statusFiltered.filter(status => status.name && status.name.toLowerCase().includes(query.toLowerCase()));
                    function compare(a: Status, b: Status) {
                        if (a.name < b.name) return -1;
                        if (a.name > b.name) return 1;
    ...
    createStatus(): Observable<Status>
    {
        ...
        return this.statuses$.pipe(
            take(1),
            switchMap(statuses => this._httpClient.post<Status>('http://localhost:1616/api/v1/followup/statuses/save', newStatus).pipe(
                map((newStatus) => {
       ...
       console.log(JSON.stringify(status));
        return this.statuses$.pipe(
            ...
            switchMap(statuses => this._httpClient.put<Status>('http://localhost:1616/api/v1/followup/statuses/statuses/' + status.id,
                ...
                switchMap(updatedStatus => this.status$.pipe(
                    take(1),
    ...
    deleteStatus(status: Status): Observable<Status>
    {
        return this.statuses$.pipe(
            take(1),
            switchMap(statuses => this._httpClient.put('http://localhost:1616/api/v1/followup/statuses/status/' + status.id, status).pipe(
    ...
    uploadAvatar(id: number, avatar: File): Observable<Status>
    {
        return this.statuses$.pipe(
            take(1),
            switchMap(statuses => this._httpClient.post<Status>('api/dashboards/statuses/avatar', {
                id
    ...
    {
        return this._httpClient.get<TypeStatus[]>('http://localhost:1616/api/v1/followup/typestatuses/all').pipe(
            tap((typeStatus) => {
