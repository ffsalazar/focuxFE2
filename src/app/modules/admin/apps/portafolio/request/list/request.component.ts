...
export class RequestListComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this._requestService.commerca$
            ...
            .subscribe((commercialArea: CommercialArea[]) => {
                ...
                // Map for clients
                this.commercialAreaSelected = this.commercialArea.map(
                    (item) => {
                        return {
                            selected: false,
                            ...item,
        ...
        this._requestService.getRequests().subscribe((response) => {
            ...
            this.requestOriginal = response.filter(
        ...
        this.filteredCommercialArea =
            this.commercialAreaControl.valueChanges.pipe(
                startWith(''),
                map((value) =>
                    typeof value === 'string' ? value : this.lastFilter
                ),
                map((filter) =>
                    this.filter(filter, this.commercialAreaSelected)
                )
            );
    ...
    private _handleChangeForm() {
        ...
        this.filterGroupForm.valueChanges.subscribe((controls) => {
            ...
            requests = requests.filter(
                (item) =>
                    (this.clientSelected.find(
                        (client) =>
                            client.selected &&
                            client.name.includes(item?.client.name)
                    ) ||
                        this.clientSelected.every(
                            (client) => !client.selected
                        )) &&
                    (this.commercialAreaSelected.find(
                        (area) =>
                            area.selected &&
                            area.name.includes(item?.commercialArea.name)
                    ) ||
                        this.commercialAreaSelected.every(
                            (area) => !area.selected
                        )) &&
                    (this.statusSelected.find(
                        (status) =>
                            status.selected &&
                            status.name.includes(item?.status.name)
                    ) ||
                        this.statusSelected.every(
                            (status) => !status.selected
                        )) &&
                    (this.businessTypeSelected.find(
                        (businessType) =>
                            businessType.selected &&
                            businessType.name.includes(
                                item?.client.businessType.name
                            )
                    ) ||
                        this.businessTypeSelected.every(
    ...
    checkerKnowledges(knowledge: Knowledge): boolean {
        let hasKnowledge = this.selectedRequest.knowledges.find(
            (selectedKnowledge) =>
                selectedKnowledge.knowledge.id === knowledge.id &&
                selectedKnowledge.isActive
        );
    ...
    addKnowledgeToCollaborator(knowledge: Knowledge): void {
        ...
        this.step2
            .get('knowledges')
            .patchValue(this.selectedRequest.knowledges);
    ...
    activeCollaboratorKnowledge(knowledge: any) {
        ...
        const knowledgeIndex = this.selectedRequest.knowledges.find(
            (item) => item.knowledge.id === knowledge.knowledge.id
        );
    ...
    toggleCollaboratorKnowledge(knowledge: Knowledge): void {
        let knowledgeFound = this.selectedRequest.knowledges.find(
            (selectedKnowledge) =>
                selectedKnowledge.knowledge.id == knowledge.id
        );

        if (knowledgeFound) {
            if (knowledgeFound.isActive)
                this.removeKnowledgeFromCollaborator(knowledgeFound);
    ...
    getAmountRequestByOption(name: string, filterOption: number) {
        switch (filterOption) {
            case 1:
                return this.requestOriginal.filter(
                    (item) => item?.client.name === name
                ).length;
                break;
            case 2:
                return this.requestOriginal.filter(
                    (item) => item?.commercialArea.name === name
                ).length;
                break;
            case 3:
                return this.requestOriginal.filter(
                    (item) => item?.status.name === name
                ).length;
                break;
            case 4:
                return this.requestOriginal.filter(
                    (item) => item?.client.businessType.name === name
                ).length;
                break;
    ...
    fillDataFormWizzard(requestId: number): void {
        ...
        this._requestService.getRequestById(requestId).subscribe((request) => {
    ...
    setRequestForm(request: Request) {
        ...
        this.step1
            .get('customerBranch')
            .setValue(request.client?.businessType.name);

        // Fill the formGroup step2
        this.step2.patchValue(request);
        this.step2
            .get('responsibleRequest')
            .setValue(request.responsibleRequest?.id || '');
        ...
        this.isToggle =
            this.step3.get('totalPauseDays').value > 0 ? true : false;
        this.hasPause =
            this.step3.get('totalPauseDays').value > 0 ? true : false;
        // Mark for check
        this._changeDetectorRef.markForCheck();
    ...
    handleChangeClients() {
        this.step1.get('client').valueChanges.subscribe((clientId) => {
            ...
            this.step1
                .get('customerBranch')
                .setValue(client?.businessType.name);
        });
    }
     ...
     */
    createRequest(): void {
        const statusId = this.status.find(
            (item) => item.name === 'Sin iniciar'
        ).id;
    ...
    createNewRequest(request: any) {
        ...
        this._requestService.createRequest(request).subscribe((newRequest) => {
            // Go to new request
    ...
    updateRequest(request: any) {
        this._requestService
            ...
            .subscribe((request) => {
                this.activatedAlert = true;
                // Show Notification and close modal
                this.showNotificationAndCloseModal(
        ...
        confirmation.afterClosed().subscribe((result) => {
            ...
            if (result === 'confirmed') {
                ...
                if (!this.isEditing) {
                    ...
                    requestNew.responsibleRequest = this.collaborators.find(
                        (item) => item.id === 1
                    );
                    requestNew.technicalArea = this.technicalArea.find(
                        (item) => item.id === requestNew.technicalArea
    ...
    optionClicked(
        event: Event,
        selectedItem: BusinessType | Client,
        selectedCollection: BusinessType[] | Client[],
        option: string
    ) {
    ...
    checkItem(item: any, collection: any) {
        return collection.find((element) => element.name === item.name) ===
            undefined
            ? false
            : true;
    }
    ...
    someComplete(collection: any, allComplete: boolean): boolean {
        return (
            collection.filter((item) => item.selected).length > 0 &&
            !allComplete
        );
    }
    ...
    updateAllComplete(option: string) {
        switch (option) {
            case 'branch':
                this.allCompleteBusinessType =
                    this.businessTypeSelected != null &&
                    this.businessTypeSelected.every((item) => item.selected);

                break;
            case 'client':
                this.allCompleteClient =
                    this.clientSelected != null &&
                    this.clientSelected.every((item) => item.selected);
                break;
            case 'area':
                this.allCompleteCommercialArea =
                    this.commercialAreaSelected != null &&
                    this.commercialAreaSelected.every((item) => item.selected);
                break;
            case 'status':
                this.allCompleteStatus =
                    this.statusSelected != null &&
                    this.statusSelected.every((item) => item.selected);
